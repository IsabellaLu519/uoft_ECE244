      ...
      player.update(input);
       ...
       for (int i = 0; i < ballCount; i++) {
        ...
        if (balls_array[i].overlap(player) != NO_OVERLAP) {
            score++;
            // Decrease player height if score is even
            if (score % 2 == 0) {
                player.decreaseHeight(1);
            }
            
            // Add a new ball if score is a multiple of 5
            if (score % 5 == 0 && ballCount < max_ballCount) {
                balls_array[ballCount] = Ball(30.0, 30.0, 0.9, 0, ballCount);
                ballCount++;
            }
        } else if (balls_array[i].getX() < 1 && 
  ...
  for (int k = 0; k < 300; k++) {
    ...
    for (int i = 0; i< ballCount; i++) {
      ...
      if (balls_array[i].overlap(player) != NO_OVERLAP) {
        ...
        //cout<<balls_array[i].overlap(player)<<"AAAA"<<balls_array[i].getX()<<endl;
        //if (balls_array[i].getX() < 1){
        //gameEnded = true;
        // cout<<"gggg"<<endl;
       // }
